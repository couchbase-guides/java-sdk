...
The `main()` method starts by initializing a connection to the cluster by calling `CouchbaseCluster.create()`. By default this method will attempt a connection to a cluster on the same machine, which is to say with 127.0.0.1 as IP.
